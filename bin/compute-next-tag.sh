...
#
# Tags look like `v<Mobilizon version>-<release>`, which is what this repository
# has always published (v3.1.0-0 ... v5.2.4-1):
#
# - if defaults/main.yml points at a Mobilizon version that has never been
#   released, the release counter restarts at 0 (`v5.2.5-0`)
# - otherwise the counter is incremented (`v5.2.5-1`), but only if something
...
# The commit-message approach this replaced only ever fired for a Renovate
# commit whose subject contained both "docker tag to " and "mobilizon", within
# the last 20 commits - so a role fix, or a version bump that scrolled out of
# that window, was never released at all.
...
# Anchored on `mobilizon_version:` so that neither the Renovate annotation
# above it nor `mobilizon_container_image_tag` /
# `mobilizon_container_image_self_build_repo_version`, which are both derived
# from it, can be mistaken for it.
version="$(sed -nE 's|^mobilizon_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Mobilizon version from $defaults_path"
	exit 1
fi

# Mobilizon's own version is carried without a leading `v` (both upstream's
# container image tags and this variable are bare), while this repository's
# tags carry one. Tolerate a `v` so that a future change of convention does not
